...
![app history](./images/app%20history.png)  
...
2. **Create a Python script for processing Delta Lake**  
    ...
    if __name__ == "__main__":
        ...
        base.write.format("delta") \\
            .mode("overwrite") \\
            .save("s3a://${SERVERLESS_BUCKET_NAME}/emr-serverless-spark/delta-lake/nyx-tlc-2021")
        spark.stop()
    EOF
    ```
...
```  
If you execute with success, you should see the similar reponse as the following:  
```sh
{
...
1. Create a workspace on the AWS Console    
2. Create an EMR cluster via the template in the workspace  
3. Check the results delivered by the EMR serverless application via an EMR notebook.  
